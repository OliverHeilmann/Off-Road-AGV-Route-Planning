...
def dijkstra_search(maze, maxvelocity, maxslope, start=(0, 0), goal=None, gridsize=(1,1)):
    if goal is None:
        goal = (len(maze) - 1, len(maze[0]) - 1)

    # f(n) = g(n) where g(n) is the total cost already accumulated called path_cost (from start
    # position), measured in total time taken to reach current position. Cost is calculated using:
    #       Time = Distance / Speed
    #______f(n)______ = _____g(n)_____
...
def dijkstraRoute3D( terrain, maxvelocity=50, maxslope=100, start=(0,0), goal=None, gridsize=(1,1) ):
    """Perform Dijkstra Search Algorithm on elevation map (using slopemap to determine obstacles) and return route as list."""
    starttime = time.time()
    final_node, number_explored = dijkstra_search( maze = terrain,
