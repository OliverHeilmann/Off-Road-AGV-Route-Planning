  ...
  while ( (read = getline(&line, &len, fp)) != -1 ){
    ...
    while ( ptr != NULL ){
      if ( row == 2 ){
        ...
        if (col == 1){ new_rotation[0] =  atof(ptr); }
        if (col == 2){ new_rotation[1] =  atof(ptr); }
        if (col == 3){ new_rotation[2] =  atof(ptr); }
        if (col == 4){ new_rotation[3] =  atof(ptr); }
      }
      ptr = strtok(NULL, " ");
      col++;
    }
...
int main(int argc, char **argv) {
  ...
  WbFieldRef translation_field = wb_supervisor_node_get_field(moose_node, "translation");
